### Inportacion de herramientas y librerias
...
###Tabla de Grubbs con un N de 10 a 30 datos
...
def graficar_distribucion(df, color = "blue"):
    fig, ax = plt.subplots()
    sns.distplot(df["Resultado"], ax=ax, color= color)
    plt.ylabel("Frecuencia")
    plt.title("Distribución de los datos")
    return fig

# Gráficas
fig_inicial = graficar_distribucion(data, color = "red")  # Llamar la función pasando el DataFrame "data" como argumento
...
st.info('Metodos de filtrado de datos, si escoge Tests de Grubbs debe elegir el nivel de confianza')
...
if boton_filtro == "Filtro Dixon":
    if st.button("Aplicar Filtro Dixon"):
        ...
        # Gráficas
        fig_dixon = graficar_distribucion(data_dixon, color='green')  # Cambiar el color a rojo
        st.subheader("Distribución despues de Dixon")
        st.pyplot(fig_dixon)
...
if boton_filtro == ("Tests Grubbs"):
    ...
    if st.button("Aplicar Tests de Grubbs"):
        ...
        fig_grubbs = graficar_distribucion(data_grubbs, color='blue')  # Cambiar el color a azul
        st.subheader("Distribución despues de Tests de Grubbs")
        st.pyplot(fig_grubbs)
...
# el valor del Z score ya sea 2 o 1.65 y se muestra en la tabla la cual despues la GUI
